...
app.MapWhen(context => context.Request.Path.StartsWithSegments("/map-when-example"), appBuilder => 
	appBuilder.UseMiddleware<MiddlewareUsedByMapWhen>());
app.UseWhen(context => context.Request.Path.StartsWithSegments("/use-when-example"), appBuilder => 
	appBuilder.UseMiddleware<MiddlewareUsedByUseWhen>());
...
public class MiddlewareFooter
{
    ...
    public async Task Invoke(HttpContext context)
    {
        context.Request.Path = "/map-when-example";
        await _next.Invoke(context);
